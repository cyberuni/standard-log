...
- b95eeeb: Bump `iso-error` from `^6.0.5` to `^7.0.0`.
...
- 6b69b99: Widen `@just-func/types` from `^0.5.0` to `^0.6.0`, and pin `type-plus` to `8.0.0-beta.10`, exactly.
...
- bd1b2e8: Move `type-plus` from `^7.0.0` to `^8.0.0-beta.10`.
...
- 0c0e0ce: Add `emitLog` support for test
...
- fdeddda: Add `getNonConsoleLogger()`.
...
- 2dd09cd: The `logLevels` values are now typed `as const` to make it easier to check when using IDE.
